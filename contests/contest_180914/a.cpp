...
int main(void){
    ...
    while (T--){
        ...
        Line line[2];
        line[0].P=(point[1]+point[3])/2;
        line[0].dir=point[3]-point[1];
        line[1].P=(point[2]+point[3])/2;
        line[1].dir=point[3]-point[2];
        func(line[0]);
        func(line[1]);
        
        Point cir=GetLineIntersection(line[0], line[1]);
        long double dirT=(cir.x-point[0].x)*(cir.x-point[0].x)+(cir.y-point[0].y)*(cir.y-point[0].y),
               dirS=(cir.x-point[1].x)*(cir.x-point[1].x)+(cir.y-point[1].y)*(cir.y-point[1].y);

        if (dirT>dirS && !equal(dirT, dirS)) printf("Accepted\n");
